...
export default async function ProductoPage({ params }: ProductoPageProps) {
  const [producto, site] = await Promise.all([
    getCatalogItemBySlug(params.slug),
    getSiteContent(),
  ])
        ...
        <div className="space-y-6">
          ...
          {!showPriceTable && (
            <p className="rounded-lg border border-heaven-divider bg-heaven-bg-card px-4 py-3 text-sm text-heaven-muted">
              Este producto no tiene precio fijo. Puedes agregarlo al carrito en la sección de productos para cotizar.
            </p>
          )}

          <ProductQuotationForm producto={producto} quoteFromQuantity={site.quoteFromQuantity} />
