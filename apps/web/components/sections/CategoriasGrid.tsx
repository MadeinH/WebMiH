      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {categorias.map((cat) => (
          <Link
            key={cat.nombre}
            href={cat.href}
            className="group rounded-[1.75rem] border border-heaven-divider bg-heaven-bg-card p-6 transition-all duration-300 hover:-translate-y-1 hover:border-heaven-lilac/30 hover:shadow-heaven-glow"
          >
            <h3 className="font-display text-xl uppercase tracking-[0.12em] text-heaven-text transition-colors duration-200 group-hover:text-heaven-lilac">
              {cat.nombre}
            </h3>
            <p className="mt-3 text-sm leading-relaxed text-heaven-muted">{cat.descripcion}</p>
            <div className="mt-4">
